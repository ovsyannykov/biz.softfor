...
@RestController
@RequestMapping(path = StdPath.ROOT + "appointment/", produces = MediaType.APPLICATION_JSON_VALUE)
@RequiredArgsConstructor
public class AppointmentCtlr {

  private final JsonFilters jsonFilters;
  private final SecurityMgr securityMgr;
  private final AppointmentSvc service;

  @RequestMapping(path = StdPath.CREATE, method = RequestMethod.POST)
  public CommonResponse<AppointmentWor> create
  (@RequestBody @Validated(Create.class) AppointmentRequest.Create request) {
  ...
  public MappingJacksonValue read(@RequestBody AppointmentRequest.Read request) {
  ...
  public CommonResponse update(@RequestBody AppointmentRequest.Update request) {
  ...
  public CommonResponse delete(@RequestBody AppointmentRequest.Delete request) {
    securityMgr.methodCheck
